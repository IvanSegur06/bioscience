...
def median(dataset, deviceCount = 0, mode = 1, debug = False):
    """
    ...
    if (dataset is not None):
        ...
        if mode == 2: # NUMBA: CPU Parallel mode
            ...
            oModel = __medianSequential(dataset, debug)
            deviceCount = 0
            sMode = "CPU Sequential"
    
    return oModel

def __medianSequential(dataset, debug):
    ...
    for pattern, value in enumerate(resultsCorrelation):
        ...
        if r1 < iRows and r2 < iRows:
            ...
            diffR1R2 = np.abs(dataset.data[r1] - dataset.data[r2])
            
            sorted_data = sorted(diffR1R2)
            if iCols % 2 == 1:
                dMedian = sorted_data[iCols // 2]
            else:
                dMedian = (sorted_data[iCols // 2 - 1] + sorted_data[iCols // 2]) / 2
        
            resultsCorrelation[pattern] = dMedian
