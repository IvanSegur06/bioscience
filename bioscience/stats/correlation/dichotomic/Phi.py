...
def phi(dataset, deviceCount = 0, mode = 1, debug = False):
    """
    Application of the Phi Coefficient.
    ...
    if (dataset is not None):
        ...
        if mode == 2: # NUMBA: CPU Parallel mode
            ...
            oModel = __phiSequential(dataset, debug)
            deviceCount = 0
            sMode = "CPU Sequential"
    
    return oModel

def __phiSequential(dataset, debug):
    ...
    for pattern, value in enumerate(resultsCorrelation):
        ...
        if r1 < iRows and r2 < iRows:
            ...
            resultsCorrelation[pattern] = phiValue
    
    if debug == True:
        end_time = time.time()
        fExecutionTime = end_time - start_time
    
    oCorrelationResults = CorrelationModel(name=PHI, results=resultsCorrelation, rows = iRows, executionTime=fExecutionTime)
